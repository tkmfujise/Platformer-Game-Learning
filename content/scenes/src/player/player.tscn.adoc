...
class N4 as "Hair" <<(N,lightskyblue) Node2D>> {
  * base_color
  * dash_color
  * color
  * segment_count
...
|===
[.node-property.node-2d]
.#$Sprites/Hair# properties
[cols="1,3" options="header"]
|===
|Name |Value
|*unique_name_in_owner* a|`true`
|*modulate* a|`Color(1, 1, 1, 0)`
|*position* a|`Vector2(2.5, 0)`
...
|Name |Value
|*unique_name_in_owner* a|`true`
|*libraries/* a|`SubResource("AnimationLibrary_u5nx7")`
|===
[.node-property.node-other]
...
const SPEED = 650.0
const DASH_SPEED = SPEED * 4.2
const CLIMBING_SPEED = SPEED / 3
