...
class N2 as "RestartCurtain" <<(C,lightgreen) Control>> {
  ~ animation_finished
---
  * pattern
  * center
...
class N3 as "Sprites" <<(N,lightskyblue) Node2D>> {
}
class N4 as "SpriteStates" <<(N,lightskyblue) Node2D>> {
}
class N5 as "IdleSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N6 as "RunSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N7 as "JumpSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N8 as "FallSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N9 as "ClimbSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N10 as "SpriteEffects" <<(N,lightskyblue) Node2D>> {
}
class N11 as "BurstSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N12 as "SweatSprite2D" <<(A,lightskyblue) AnimatedSprite2D>> {
}
class N13 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N14 as "AreaCollision" <<(A,lightskyblue) Area2D>> {
}
class N15 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N16 as "FrameSubject" <<(A,lightskyblue) Area2D>> {
}
class N17 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N18 as "FrontRayCast" <<(R,lightskyblue) RayCast2D>> {
}
class N19 as "Camera" <<(C,lightskyblue) Camera2D>> {
  - DASH_SHAKING_TIME
...
class N21 as "AnimationTree" <<(A,yellow) AnimationTree>> {
}
R --o N1
R --o N3
R --o N13
...
_(R::_on_area_collision_area_entered, N14::area_entered)
_(R::_on_frame_subject_area_entered, N16::area_entered)
_(R::_on_frame_subject_area_exited, N16::area_exited)
....
...
const CLIMBING_SPEED = SPEED / 5
const JUMP_VELOCITY = -1100.0
const GRAVITY = 4000
const DOWNFORCE_ON_LITTLE_JUMP = GRAVITY * 7
const MAX_VELOCITY_Y = 1300
...
func compute_state(delta: float, direction: Vector2) -> State:
	...
	and direction:
		...
		if not is_on_floor():
			if on_wall \
			and (%FrontRayCast.target_position.x * direction.x > 0):
				falling_time = 0
...
func regain_dash_charge() -> void:
	dash_rest_count = [MAX_DASH_COUNT, dash_rest_count + 1].min()
	climb_rest_time = MAX_CLIMBING_TIME
	vibrate(&'Charge')
...
func vibrate(key: StringName):
	var values = [0, 0, 0]
	match key:
		&'Dashed': values = [0.4, 0.6, 0.2]
		&'Landed': values = [0.1, 0.3, 0.1]
		&'Charge': values = [0.2, 0.4, 0.2]
...
func get_state_by(_velocity) -> String:
	...
	if state == State.SLIDING: return &'Climb'
	if _velocity:
