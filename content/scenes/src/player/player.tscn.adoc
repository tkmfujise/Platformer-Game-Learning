...
class N10 as "SweatSprite2D" <<(A,lightskyblue) AnimatedSprite2D>> {
}
class N11 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N12 as "AreaCollision" <<(A,lightskyblue) Area2D>> {
}
class N13 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N14 as "FrameSubject" <<(A,lightskyblue) Area2D>> {
}
class N15 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N16 as "FrontRayCast" <<(R,lightskyblue) RayCast2D>> {
}
class N17 as "RestartCurtain" <<(C,lightgreen) Control>> {
  ~ animation_finished
---
  * pattern
  * center
...
class N18 as "Camera" <<(C,lightskyblue) Camera2D>> {
  - DASH_SHAKING_TIME
...
class N20 as "AnimationTree" <<(A,yellow) AnimationTree>> {
}
R --o N1
R --o N11
R --o N12
...
_(R::_on_failure_collision_area_entered, N12::area_entered)
_(R::_on_frame_subject_area_entered, N14::area_entered)
_(R::_on_frame_subject_area_exited, N14::area_exited)
....
...
func _on_failure_collision_area_entered(area: Area2D) -> void:
	print_debug('area.collision_layer = %s' % area.collision_layer)
	failed.emit()
```
[.signal-connection]
