...
class N7 as "ClimbSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N8 as "SpriteEffects" <<(N,lightskyblue) Node2D>> {
}
class N9 as "BurstSprite2D" <<(S,lightskyblue) Sprite2D>> {
}
class N10 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N11 as "AreaCollision" <<(A,lightskyblue) Area2D>> {
}
class N12 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N13 as "FrameSubject" <<(A,lightskyblue) Area2D>> {
}
class N14 as "CollisionShape2D" <<(C,lightskyblue) CollisionShape2D>> {
}
class N15 as "RestartCurtain" <<(C,lightgreen) Control>> {
  ~ animation_finished
---
  * pattern
  * center
...
class N16 as "Camera" <<(C,lightskyblue) Camera2D>> {
  - DASH_SHAKING_TIME
  * shaking_time
  * fixed_position
  * current_frames
...
class N18 as "AnimationTree" <<(A,yellow) AnimationTree>> {
}
R --o N1
R --o N10
R --o N11
...
_(R::_on_failure_collision_area_entered, N11::area_entered)
_(R::_on_frame_subject_area_entered, N13::area_entered)
_(R::_on_frame_subject_area_exited, N13::area_exited)
....
...
func compute_state(delta: float, direction: Vector2) -> State:
	...
	and direction:
		...
		# FIXME: 壁のてっぺん ┌ の形のあたりでがたがたする。RayCast を使う？
		var on_wall = test_move(global_transform, Vector2(facing_direction, 0))
...
func animate_with(_velocity: Vector2) -> void:
	if _velocity.x:
		facing_direction = sign(_velocity.x)
...
func update_sprite_states_shader_parameters() -> void:
	for sprite in %SpriteStates.get_children():
		...
		var fatigue = 0.0
		if state == State.CLIMBING: fatigue = climb_rest_time / MAX_CLIMBING_TIME
