...
func _on_finished() -> void:
	stop()
	#queue_free() FIXME ReDScribe を複数呼び出しているときに参照エラーになる
```
[.signal-connection]
.$.:[#_paused_#]=>$.
```gdscript
...
|*custom_effects* a|`[SubResource("RichTextEffect_s5vpw")]`
|*script* a|link:/scripts/src/dialogue/content.gd[res://src/dialogue/content.gd]
|===
[.node-property.node-other]
...
|Name |Value
|*unique_name_in_owner* a|`true`
|*libraries/* a|`SubResource("AnimationLibrary_5m8cj")`
|*autoplay* a|`&"ready"`
|===
...
const DISPLAY_CHAR_INTERVAL := 0.05
var controller : ReDScribe
var current_speaker : String = ''
var waiting : bool = false
static var current : Dialogue


func setup_redscribe() -> void:
	controller = ReDScribe.new()
...
func start() -> void:
	current = self
	if not controller: setup_redscribe()
	show()
	continue_dialogue()
	started = true
	waiting = false
...
func show_content(content: String) -> void:
	...
	%Content.text = "[dialogue]%s[/dialogue]" % content
	var duration = %Content.get_parsed_text().length() * DISPLAY_CHAR_INTERVAL
	tween.tween_property(%Content, 'visible_ratio', 1.0, duration)
