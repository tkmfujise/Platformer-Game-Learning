...
func _physics_process(delta: float) -> void:
	if dash_rest_time > 0:
		...
		# Handle jump.
		if playable and Input.is_action_just_pressed("move_jump") and falling_time < ALLOWED_TIME_TO_JUMP:
			velocity.y = JUMP_VELOCITY

		var direction_x := Input.get_axis("move_left", "move_right")
		if playable and direction_x:
			velocity.x = direction_x * SPEED
		else:
			velocity.x = move_toward(velocity.x, 0, SPEED)

		# Handle dash.
		if playable and Input.is_action_just_pressed("move_dash") && dash_rest_count > 0:
