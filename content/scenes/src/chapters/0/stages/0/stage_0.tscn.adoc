...
class N26 as "Player" <<(C,lightskyblue) CharacterBody2D>> {
  ~ dashed
  ~ jumped
  ~ landed
  ~ failed
...
class N27 as "Weather" <<(N,lightskyblue) Node2D>> {
---
  + _process()
  + current_camera()
}
class N28 as "Transitionable" <<(N,darkgray) Node>> {
  * menu_scene
---
  + transit()
}
class N29 as "AnimationPlayer" <<(A,yellow) AnimationPlayer>> {
}
R --o N1
R --o N4
R --o N5
...
_(R::_on_player_failed, N26::failed)
....
