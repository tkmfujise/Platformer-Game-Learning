...
class N13 as "Gimmicks" <<(N,lightskyblue) Node2D>> {
}
class N14 as "FallingIceBlock" <<(R,lightskyblue) RigidBody2D>> {
---
  + _ready()
  + _process()
  + fall()
...
class N15 as "Player" <<(C,lightskyblue) CharacterBody2D>> {
  ~ dashed
  ~ failed
---
  - SPEED
...
class N16 as "Weather" <<(N,lightskyblue) Node2D>> {
---
  + _process()
  + current_camera()
}
class N17 as "AnimationPlayer" <<(A,yellow) AnimationPlayer>> {
}
class N18 as "House" <<(S,lightskyblue) Sprite2D>> {
}
R --o N1
R --o N4
R --o N5
...
_(R::_on_player_failed, N15::failed)
....
