...
[plantuml]
....
class ROOT as "Stage0" <<Control>> {
---
}
class OpeningCurtain as "OpeningCurtain" <<Control>> {
  * pattern
  * center
---
  + set_center()
...
class Forest as "Forest" <<Node2D>> {
}
class Frames as "Frames" <<Node2D>> {
}
class Frames_CameralFrame as "CameralFrame" <<Area2D>> {
  - HALF_SIZE
---
  + corner()
}
class Frames_CameraFrame2 as "CameraFrame2" <<Area2D>> {
  - HALF_SIZE
---
  + corner()
}
class Frames_CameraFrame3 as "CameraFrame3" <<Area2D>> {
  - HALF_SIZE
---
  + corner()
}
class StageGround0 as "StageGround0" <<TileMapLayer>> {
}
class Gimmicks as "Gimmicks" <<Node2D>> {
}
class Gimmicks_FallingIceBlock as "FallingIceBlock" <<RigidBody2D>> {
---
  + _ready()
  + fall()
  + stop()
...
class FallZones as "FallZones" <<Node2D>> {
}
class FallZones_FallZone as "FallZone" <<Node2D>> {
}
class Player as "Player" <<CharacterBody2D>> {
  ~ dashed
---
  - SPEED
  - DASH_SPEED
...
class Weather as "Weather" <<Node2D>> {
---
  + _process()
  + current_camera()
}
...
FallZones --o FallZones_FallZone
...
** [.node-name.type-2d]#FallZones# [.node-type.type-2d]#Node2D#
*** [.node-name.type-2d]#FallZone# [.node-type.type-2d]#link:/scenes/src/stages/fall_zone/fall_zone.tscn[res://src/stages/fall_zone/fall_zone.tscn]#
...
|_node_ |*position* |`Vector2(4888, 1260)`
|_node_ |*scale* |`Vector2(20, 1)`
|_node_ |*position* |`Vector2(328, 284)`
|_node_ |*position* |`Vector2(26, 1168)`
